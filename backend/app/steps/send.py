...
from ..db import email_already_sent, mark_email_sent
...
def run(self, lead_id: str) -> str:
    ...
    if lead.status in (LeadStatus.DEAD, LeadStatus.SENT):
        return lead_id
    ...
    if email_already_sent(lead.verified_email):
        emit(lead_id, "send", f"Duplicate: {lead.verified_email} already received an email; skipping", level="warn")
        set_status(lead_id, LeadStatus.DEAD)
        return lead_id
    ...
    mark_email_sent(lead.verified_email)
